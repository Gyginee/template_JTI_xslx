...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["excelFile"])) {
    ...
    if (in_array($uploadedFileExtension, $allowedExtensions)) {
        ...
        if (move_uploaded_file($uploadedFile['tmp_name'], $tempFilePath)) {
            ...
            SET winnerName = NULL, 
                winnerBankName = NULL, 
                winnerBankNumber = NULL, 
                winnerRelationship = NULL 
            WHERE storeCode = ?;
            ";
            ...
            for ($row = 2; $row <= $maxRow; $row++) {
                $storeId = $sheet->getCell($columns['storeCode'] . $row)->getValue();

                // Bind parameters and execute the statement
                $stmt->bind_param('s', $storeId);
                if ($stmt->execute()) {
                    $results[] = ['storeCode' => 'Lười check', 'StoreId' => $storeId, 'Updated' => 'Delete Reward'];
                } else {
